        ...
        if(GlobalController.Instance.demoDragType == DragType.throw_catch)
        {
            if (GlobalController.Instance.demoTarget2Status == TargetStatus.total_on_screen_1)
            {
                    ...
                    && GlobalController.Instance.demoTarget1ThrowCatchStatus == ThrowCatchStatus.throw_successed_on_screen_1) 
                {
                    ...
                    else if (Input.GetMouseButtonUp(0))
                    {
                        ...
                        target2.transform.position = processScreenPosToGetWorldPosAtZeroZ(Input.mousePosition);
                        curTarget2ThrowCatchStatus = ThrowCatchStatus.catched_on_screen_2;
                    }
                    ...
                    if ( Input.touchCount == 1 )
                    {
                        ...
                        {
                            touchVisualizer.hideTouchOutline();
                            target2.transform.position = processScreenPosToGetWorldPosAtZeroZ(touch.position);
                            curTarget2ThrowCatchStatus = ThrowCatchStatus.catched_on_screen_2;
                        }
                    } 
#endif
                } 
                else if (curTarget2ThrowCatchStatus == ThrowCatchStatus.catched_on_screen_2)
                {
            ...
            else if (GlobalController.Instance.demoTarget2Status == TargetStatus.total_on_screen_2)
            {
                ...
                else if (curTarget2ThrowCatchStatus == ThrowCatchStatus.throw_flicked_on_screen_2)
                {
                    ...
                    curTarget2ThrowCatchStatus = ThrowCatchStatus.wait_for_t2_move;
                }
                else if (curTarget2ThrowCatchStatus == ThrowCatchStatus.wait_for_t2_move)
                {
                ...
                {
                    if (GlobalController.Instance.demoTarget1ThrowCatchStatus == ThrowCatchStatus.catched_on_screen_1)
                    {
                        curTarget2ThrowCatchStatus = ThrowCatchStatus.catched_on_screen_1;
                    }
                }
                else if (curTarget2ThrowCatchStatus == ThrowCatchStatus.catched_on_screen_1)
                {
            ...
            if (curTarget2ThrowCatchStatus != prevTarget2ThrowCatchStatus &&
                 (  curTarget2ThrowCatchStatus == ThrowCatchStatus.catched_on_screen_2
                 || curTarget2ThrowCatchStatus == ThrowCatchStatus.throw_successed_on_screen_2)
               )
            {
