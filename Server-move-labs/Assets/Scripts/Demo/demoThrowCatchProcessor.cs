...
public class demoThrowCatchProcessor : MonoBehaviour
{
    ...
    private float throwStartTime, throwEndTime, moveStartTime;
    ...
    void Update()
    {
        if(!GlobalController.Instance)
        {
            ...
            else if (curTarget1ThrowCatchStatus == ThrowCatchStatus.throw_flicked_on_screen_1)
            {
                ...
                moveEndPos = target1.transform.position + (throwEndPos - throwStartPos);
                ...
                curTarget1ThrowCatchStatus = ThrowCatchStatus.wait_for_t1_move;
            }
            else if (curTarget1ThrowCatchStatus == ThrowCatchStatus.wait_for_t1_move)
            {
                float t = (Time.time - throwStartTime) / thisMoveDuration;
                ...
                {
                    target1.transform.position = calculate3rdPointOnLine(throwStartPos, throwEndPos);
                }
        ...
        else if (GlobalController.Instance.demoDragType == DragType.throw_catch)
        {
            if (GlobalController.Instance.demoTarget1Status == TargetStatus.total_on_screen_1)
            {
                ...
                {
                    if (GlobalController.Instance.demoTarget2ThrowCatchStatus == ThrowCatchStatus.catched_on_screen_2)
                    {
                        curTarget1ThrowCatchStatus = ThrowCatchStatus.catched_on_screen_2;
                    }
                }
                else if (curTarget1ThrowCatchStatus == ThrowCatchStatus.catched_on_screen_2)
                {
            ...
            else if (GlobalController.Instance.demoTarget1Status == TargetStatus.total_on_screen_2)
            {
                    ...
                    && GlobalController.Instance.demoTarget2ThrowCatchStatus == ThrowCatchStatus.throw_successed_on_screen_2)
                {
                    ...
                    else if (Input.GetMouseButtonUp(0))
                    {
                        ...
                        target1.transform.position = processScreenPosToGetWorldPosAtZeroZ(Input.mousePosition);
                        curTarget1ThrowCatchStatus = ThrowCatchStatus.catched_on_screen_1;
                    }
                    else if (Input.GetMouseButton(0))
                    {
                        touchVisualizer.adjustTouchOutlinePosition(Input.mousePosition);
                    ...
                    if ( Input.touchCount == 1 )
                    {
                        ...
                        {
                            touchVisualizer.hideTouchOutline();
                            target1.transform.position = processScreenPosToGetWorldPosAtZeroZ(touch.position);
                            curTarget1ThrowCatchStatus = ThrowCatchStatus.catched_on_screen_1;
                        }
                    } 
#endif
                }
                else if (curTarget1ThrowCatchStatus == ThrowCatchStatus.catched_on_screen_1)
                {
        ...
        if (curTarget1ThrowCatchStatus != prevTarget1ThrowCatchStatus &&
             (  curTarget1ThrowCatchStatus == ThrowCatchStatus.throw_successed_on_screen_1
             || curTarget1ThrowCatchStatus == ThrowCatchStatus.catched_on_screen_1)
           )
        {
